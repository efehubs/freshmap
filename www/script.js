...
const serverGametypeInput = /** @type {HTMLInputElement} */ (document.getElementById('server-gametype-input'));
...

{
  ...
  const serverGametype = localStorage.getItem('serverGametype') ?? serverId.substring(0, 8);
  localStorage.setItem('serverGametype', serverGametype);
  serverGametypeInput.value = serverGametype;
...
serverGametypeInput.addEventListener('change', () => {
  if (serverGametypeInput.value !== '') {
    localStorage.setItem('serverGametype', serverGametypeInput.value);
    pushSettings();
  }
});
...
window.addEventListener('storage', () => {
  ...
  serverGametypeInput.value = localStorage.getItem('serverGametype');
...
async function pushSettings() {
  ...
  try {
    response = await fetch('update-settings?' + new URLSearchParams({
      ...
      server_gametype: localStorage.getItem('serverGametype')
    }));
  } catch (error) {
